...
class OrderRepository{
	public function makeOrder($parameters){
		...
		$cart = optional((new Cart())->where(['customer_id' => $customerIdentity->id])->get()->first())->toArray();
		$cartItems = (new CartItems())->where(['customer_id' => $customerIdentity->id])->get()->toArray();

		# order
		$order_id = $this->generateOrderId($cart['cart_id']);
		$sql = "insert into orders (cart_id,order_id,customer_id,amount,shipping_cost,net_amount,is_cod,payment_platform) select cart_id,'{$order_id}',customer_id,amount,shipping_cost,net_amount,'{$is_cod}','{$payment_platform}' from cart where customer_id = '{$customerIdentity->id}'";
		DB::insert($sql);

		#order items
		foreach ($cartItems as $key => $value) {
			$itemsSql = "insert into orders_items (cart_id,order_id,customer_id,product_id,mrp,category_code,sub_category_code,product_code,brand_code,name,images,image_base_path,variants,attributes,offer_value,offer_type,quantity,product_amount,total_amount) values('{$value['cart_id']}','{$order_id}','{$value['customer_id']}','{$value['product_id']}','{$value['mrp']}','{$value['category_code']}','{$value['sub_category_code']}','{$value['product_code']}','{$value['brand_code']}','{$value['name']}','[]','{$value['image_base_path']}','{$value['variants']}','{$value['attributes']}','{$value['offer_value']}','{$value['offer_type']}','{$value['quantity']}','{$value['product_amount']}','{$value['total_amount']}')";
			Log::warning($itemsSql);
			DB::insert($itemsSql);
		}

		# remove cart and cart_items
		(new Cart())->where(['customer_id' => $customerIdentity])->delete();
		(new CartItems())->where(['customer_id' => $customerIdentity])->delete();
	}
